...
class GetIDList:
    def __init__(self, address, url,a):
    ...
    def getidlist(self):
        ...
        if self.a:
            self.address = self.address[self.a:]
        num = 26
        for i in self.address:
            ...
            while num<page:
                ...
                for x in res["express_search"]:
                    ...
                    GetDetails(url, x["realUid"]).getdatails(self.db,self.cursor)
                self.db.commit()
                print("提交提交提交提交提交提交提交提交提交提交提交提交提交")
                for j in res["userInfo"]:
                    url = "http://www.jiayuan.com/" + str(j["uid"])
                    GetDetails(url, j["realUid"]).getdatails(self.db,self.cursor)
                # del1 = "delete from count"
                # self.cursor.execute(del1)
                #
                # sele = "insert into count values (%s,%s)"
                # self.cursor.execute(sele, (i, num))
                self.db.commit()
                print("提交提交提交提交提交提交提交提交提交提交提交提交提交")
                num += 1
            num = 0
...
class GetDetails:
    ...
    def getdatails(self,db,cursor):
        ...
        SaveText(db,cursor).save(y_age, y_address, y_education, y_marital_status, y_salary, y_stuture, name, age, money, stature,
...
if __name__ == '__main__':
    ...
    def a(result, url,a):
        GetIDList(result,url,a).getidlist()
    num=0
    for i in range(37):
        threading.Thread(target=a,args=(result,url,num)).start()
        # print(num)
        num+=1